...
import static in.projecteka.library.clients.model.ErrorCode.BAD_REQUEST_FROM_GATEWAY;
import static in.projecteka.library.clients.model.ErrorCode.CONSENT_ARTEFACT_EXPIRED;
import static in.projecteka.library.clients.model.ErrorCode.CONSENT_ARTEFACT_FORBIDDEN;
import static in.projecteka.library.clients.model.ErrorCode.CONSENT_ARTEFACT_NOT_FOUND;
import static in.projecteka.library.clients.model.ErrorCode.CONSENT_NOT_GRANTED;
import static in.projecteka.library.clients.model.ErrorCode.CONSENT_REQUEST_EXPIRED;
import static in.projecteka.library.clients.model.ErrorCode.CONSENT_REQUEST_NOT_FOUND;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_DATE_RANGE;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_LINK_REFERENCE;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_OTP_ATTEMPTS_EXCEEDED;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_PIN_ATTEMPTS_EXCEEDED;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_PROVIDER_OR_CARE_CONTEXT;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_RECOVERY_REQUEST;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_REQUEST;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_REQUESTER;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_RESP_REQUEST_ID;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_SCOPE;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_SESSION;
import static in.projecteka.library.clients.model.ErrorCode.INVALID_TOKEN;
import static in.projecteka.library.clients.model.ErrorCode.NETWORK_SERVICE_ERROR;
import static in.projecteka.library.clients.model.ErrorCode.NO_PATIENT_FOUND;
import static in.projecteka.library.clients.model.ErrorCode.NO_RESULT_FROM_GATEWAY;
...
import static in.projecteka.library.clients.model.ErrorCode.OTP_REQUEST_LIMIT_EXCEEDED;
import static in.projecteka.library.clients.model.ErrorCode.PROVIDER_NOT_FOUND;
import static in.projecteka.library.clients.model.ErrorCode.QUEUE_NOT_FOUND;
import static in.projecteka.library.clients.model.ErrorCode.REFRESH_TOKEN_INCORRECT;
import static in.projecteka.library.clients.model.ErrorCode.REQUEST_ALREADY_EXISTS;
import static in.projecteka.library.clients.model.ErrorCode.TRANSACTION_ID_NOT_FOUND;
import static in.projecteka.library.clients.model.ErrorCode.TRANSACTION_PIN_IS_ALREADY_CREATED;
import static in.projecteka.library.clients.model.ErrorCode.TRANSACTION_PIN_NOT_FOUND;
import static in.projecteka.library.clients.model.ErrorCode.UNABLE_TO_CONNECT_TO_PROVIDER;
import static in.projecteka.library.clients.model.ErrorCode.UNKNOWN_ERROR_OCCURRED;
import static in.projecteka.library.clients.model.ErrorCode.UNPROCESSABLE_RESPONSE_FROM_GATEWAY;
import static in.projecteka.library.clients.model.ErrorCode.USERNAME_OR_PASSWORD_INCORRECT;
...
public class ClientError extends Throwable {
    ...
    public static ClientError transactionPinDidNotMatch(String auxMessage) {
        return new ClientError(UNAUTHORIZED,
                new ErrorRepresentation(new Error(ErrorCode.INVALID_TRANSACTION_PIN,
                        format("%s;%s", INVALID_TRANSACTION_PIN, auxMessage))));
    }

    public static ClientError invalidAttemptsExceeded() {
        return new ClientError(UNAUTHORIZED,
                new ErrorRepresentation(new Error(INVALID_PIN_ATTEMPTS_EXCEEDED,
                        "Invalid Pin attempts exceeded; Try again after sometime.")));
    }

    public static ClientError invalidTransactionPin() {
        return new ClientError(BAD_REQUEST,
                new ErrorRepresentation(new Error(ErrorCode.INVALID_TRANSACTION_PIN, INVALID_TRANSACTION_PIN)));
    }
    ...
    public static ClientError consentArtefactNotFound() {
        return new ClientError(NOT_FOUND,
                new ErrorRepresentation(new Error(CONSENT_ARTEFACT_NOT_FOUND, CANNOT_FIND_THE_CONSENT_ARTEFACT)));
    }

    public static ClientError consentArtefactForbidden() {
        return new ClientError(FORBIDDEN,
                new ErrorRepresentation(new Error(CONSENT_ARTEFACT_FORBIDDEN, CANNOT_FIND_THE_CONSENT_ARTEFACT)));
    }
    ...
    public static ClientError invalidScope() {
        return new ClientError(UNAUTHORIZED,
                new ErrorRepresentation(new Error(INVALID_SCOPE,
                        "The scope provided is invalid for current operation")));
    }

    public static ClientError invalidSession(String session) {
        return new ClientError(BAD_REQUEST,
                new ErrorRepresentation(new Error(INVALID_SESSION, format("The sessionId: %s is invalid", session))));
    }

    public static ClientError gatewayTimeOut() {
        return new ClientError(GATEWAY_TIMEOUT,
                new ErrorRepresentation(new Error(NO_RESULT_FROM_GATEWAY,
                        "Didn't receive any result from Gateway")));
    }

    public static ClientError tooManyInvalidOtpAttempts() {
        return new ClientError(TOO_MANY_REQUESTS,
                new ErrorRepresentation(new Error(INVALID_OTP_ATTEMPTS_EXCEEDED,
                        "Invalid OTP attempts limit exceeded")));
    }
    ...
    public static ClientError invalidResponseFromHIP() {
        return new ClientError(UNPROCESSABLE_ENTITY,
                new ErrorRepresentation(new Error(UNPROCESSABLE_RESPONSE_FROM_GATEWAY,
                        "Could not process response from HIP")));
    }

    public static ClientError invalidOldPassword(int remainingAttempts) {
        return new ClientError(UNAUTHORIZED, new ErrorRepresentation(new Error(USERNAME_OR_PASSWORD_INCORRECT,
                "You have " + remainingAttempts + " tries available after that your account will be locked")));
    }
