...
import in.projecteka.user.filters.ABPMJAYIdFilter;
import in.projecteka.user.filters.NameFilter;
import in.projecteka.user.filters.YOBFilter;
...
import in.projecteka.user.model.Patient;
...
import static in.projecteka.user.model.LoginMode.OTP;
...
public class UserService {
    ...
    public Mono<User> userWith(String userName) {
        return userRepository.userWith(userName.toLowerCase()).switchIfEmpty(Mono.error(ClientError.userNotFound()));
    }
    ...
    public Mono<LoginModeResponse> getLoginMode(String userName) {
        return tokenService.tokenForAdmin()
                ...
                .map(discard -> LoginModeResponse.builder().loginMode(CREDENTIAL).build())
                .switchIfEmpty(Mono.just(LoginModeResponse.builder().loginMode(OTP).build()));
    }
